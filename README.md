...
comments from collaborators, update the Quarto file, and repeat. Changes
do not sync back from Google Docs to Quarto. The Quarto file remains the
source of truth. This lets collaborators comment and suggest changes in
Google Docs. The lead researcher then reviews the suggestions and
manually transfers accepted changes back into the Quarto file.
...
pubthis uses a `_publish_ids.yml` file next to your `.qmd` to remember
which Google Doc to update. Commit this file so everyone on the project
opens the same doc. There are two ways to set this up:

1.  Let pubthis create a new doc: On first publish, a new Google Doc is
    created and `_publish_ids.yml` is written automatically. Every
    subsequent `publish` call updates that same doc in place.
2.  Use an existing doc: If a shared doc already exists, create
    `_publish_ids.yml` manually and paste in the doc ID (the long string
    between `/d/` and `/edit` in the doc URL)
